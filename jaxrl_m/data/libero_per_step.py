    ...
    def process_episode(self, episode_dir, language_instructions, demo_data, episode_index, episode_index_in_task):
        ...
        for step_index in range(num_steps):
            ...
            with h5py.File(f'{step_dir}/other.h5', 'w') as h5_file:
                ...
                observation_group.create_dataset(name='ee_pos', data=ee_pos[step_index])
                observation_group.create_dataset(name='ee_ori', data=ee_ori[step_index])
...
def main(rank, port, num_worker, start_episode_idx=0, end_episode_idx=None):
    ...
    src_dir = f"/data/hf_cache/datasets/LIBERO/{dataset_name}"
    tgt_dir = Path(f"/data/hf_cache/datasets/LIBERO/{dataset_name}_converted")
